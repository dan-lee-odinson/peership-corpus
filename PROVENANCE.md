...
**One paper's record is shaped differently, and it is stated rather than glossed.** *The Isonomia Commons* (III) was reviewed hard — five documented formal adversarial cycles against the whitepaper it condenses, plus a publication audit that corrected substantive overclaims in the preprint itself. But it is the **least checkable** of the five: its reviewer prompt was never preserved (what is published is a *reconstruction*, not a transcript), its verbatim review texts are held pending the ISONOMIA repository's scheduled release, and most of that scrutiny fell on the design and the package rather than on the essay as an argument. Papers II and V, by contrast, publish their **actual** prompts. A reader should weight paper III accordingly. The full account is in [`reviews/REVIEW_PROVENANCE.md`](reviews/REVIEW_PROVENANCE.md).
