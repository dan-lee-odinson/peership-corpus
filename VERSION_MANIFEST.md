...
| `peership_claim_ledger.md` (apparatus) | Claim-level provenance; 70 logged claims (the file's own summary says "~71" — erratum E2 in apparatus/corrections_log.md) (CL-F#, CL-D#, CL-G#, CL-P#, CL-I#, CL-C#, CL-T#) | v1.2 header, revision 1.3 (deposit day) — see [Apparatus versions](#apparatus-versions) | Live apparatus, synchronized | *No DOI of its own* — archived within record 21359124 (release zip) | *(none of its own)* | [apparatus/peership_claim_ledger.md](apparatus/peership_claim_ledger.md) | CC BY 4.0 |

### Licence split

All scholarly material in this repository — the papers, the PDFs, the Markdown, the apparatus, the bibliography, and the documentation — is **CC BY 4.0** ([LICENSE](LICENSE)). The Apache-2.0 licence in [LICENSE-APACHE](LICENSE-APACHE) is reserved for executable code and build scripts; **this repository currently contains no executable code.** The papers are not Apache-licensed. The file-level table above controls.
