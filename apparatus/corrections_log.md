...
| **ISONOMIA manifest DOI label** | A **mislabelling, not a bad DOI.** The [`isonomia-path-a`](https://github.com/dan-lee-odinson/isonomia-path-a) repository's `VERSION_MANIFEST.md` calls `10.5281/zenodo.21338480` the preprint's **concept DOI**. It is not — it is the **v1.0 exact-version DOI**, and it remains valid and permanent, correct to cite whenever v1.0 is what is meant. The concept (all-versions) DOI is `…21338479`. Only the word *concept* attached to `…21338480` is wrong. Resolved by ledger **CL-D3**; recorded as discrepancy **D5** in [`../VERSION_MANIFEST.md`](../VERSION_MANIFEST.md). | A one-word correction to that repository's manifest. **Deferred:** `isonomia-path-a` is **frozen** until a full release is scheduled, so the fix travels with that release. This corpus already records the correct reading, so no citation here is affected. |
| **ISONOMIA release-package reviews** | The adversarial review *notes* on the ISONOMIA whitepaper, launch spec, simulation plan and calibration record are **held unpublished**, pending the frozen `isonomia-path-a` repository's scheduled release. The audit *trails* are published ([`../reviews/03-isonomia-commons/`](../reviews/03-isonomia-commons/)) and the *dispositions* are public in that repository, so the review is evidenced. What a reader cannot yet do is read the adversary's own text and judge whether the dispositions were adequate. | Publication of the review notes with the ISONOMIA release. |
| **ISONOMIA reviewer prompt** | The original adversarial-review prompt for the ISONOMIA whitepaper **was never preserved**. The audit trail publishes a **reconstruction**, and labels it as such. This is a genuine provenance deficit: papers II and V publish verbatim prompts, so a reader can confirm what the reviewer was told to do; for paper III they cannot. | Recovery of the original prompt, if session records are ever archived. Until then the reconstruction must **never** be cited as a transcript. |
| **Audit-trail DOI label** | The same mislabelling as above, in [`../reviews/03-isonomia-commons/whitepaper_and_preprint_audit_trail.md`](../reviews/03-isonomia-commons/whitepaper_and_preprint_audit_trail.md) §13: it captions `10.5281/zenodo.21338480` "preprint concept DOI." That DOI is valid — it is the **v1.0 exact-version** DOI — but the caption is wrong; the concept DOI is `…21338479` (Zenodo API capture, 2026-07-14; ledger **CL-D3**). | The document is committed **as supplied**, with an editorial note at its head recording the correction, rather than silently patched. Corrected at the next revision of that document. |
