...
| **ISONOMIA release-package reviews** | Adversarial review notes on the ISONOMIA whitepaper, launch spec, simulation plan and calibration record exist but are **held unpublished**, pending the frozen `isonomia-path-a` repository's scheduled release. Until then, the claim that ISONOMIA's whitepaper was extensively reviewed upstream is the author's account and is **not reader-verifiable**. | Publication of those notes with the ISONOMIA release. See [`../reviews/REVIEW_PROVENANCE.md`](../reviews/REVIEW_PROVENANCE.md) under paper III. |
