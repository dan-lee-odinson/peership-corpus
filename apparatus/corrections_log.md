...
| **ISONOMIA release-package reviews** | The adversarial review *notes* on the ISONOMIA whitepaper, launch spec, simulation plan and calibration record are **held unpublished**, pending the frozen `isonomia-path-a` repository's scheduled release. The *dispositions* are already public and dated in that repository — the whitepaper changelog's "adversarial-review response" entry, the launch-spec changelog's claim-discipline pass, `CALIBRATION.md`'s revision note, and `DECISIONS.md` #35. So the upstream review is evidenced; what a reader cannot yet do is read the reviews and judge whether the dispositions were adequate. | Publication of the review notes with the ISONOMIA release. See [`../reviews/REVIEW_PROVENANCE.md`](../reviews/REVIEW_PROVENANCE.md) under paper III. |
