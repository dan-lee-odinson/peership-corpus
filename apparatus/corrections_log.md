...
| **ISONOMIA manifest DOI** | The separate [`isonomia-path-a`](https://github.com/dan-lee-odinson/isonomia-path-a) repository's `VERSION_MANIFEST.md` still labels `10.5281/zenodo.21338480` the preprint's **concept DOI**. It is not: the Zenodo API gives `…21338479` as the concept DOI and `…21338480` as the **v1.0 exact-version DOI**. Resolved in this corpus by ledger **CL-D3** and recorded as discrepancy **D5** in [`../VERSION_MANIFEST.md`](../VERSION_MANIFEST.md); the other repository was never updated to match. | A correction to that repository's manifest. **Deferred by author decision:** `isonomia-path-a` is **frozen** until a full release is scheduled, so this fix is carried forward to that release rather than applied piecemeal now. This corpus already records the correct reading, so no citation here is affected. |
...
| **Audit-trail DOI erratum** | [`../reviews/03-isonomia-commons/whitepaper_and_preprint_audit_trail.md`](../reviews/03-isonomia-commons/whitepaper_and_preprint_audit_trail.md) §13 gives the preprint **concept DOI** as `10.5281/zenodo.21338480`. It is not — `…21338479` is the concept DOI and `…21338480` is the **v1.0 exact-version** DOI (Zenodo API capture, 2026-07-14; ledger **CL-D3**). The same error as the ISONOMIA manifest's (see D5). | The document is committed **as supplied**, with an editorial note at its head recording the correction, rather than silently patched. Corrected at the next revision of that document. |
